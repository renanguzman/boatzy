...
/**
 * Chat cliente ↔ dono a partir do detalhe da embarcação. Reutiliza a conversa
 * única gestor↔cliente (§21); a primeira mensagem vem pré-preenchida citando
 * a embarcação.
 */
export default async function ChatEmbarcacaoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (embarcacao.owner_id === user.id) redirect(`/embarcacoes/${id}`);

  const { data: dono } = await supabaseAdmin
    .from('users')
    .select('id, name, email, avatar_url')
    .eq('id', embarcacao.owner_id)
    .single();
  if (!dono) notFound();

  // Garante (idempotente) a conversa gestor ↔ cliente, registrando a origem
  // (última a abrir o chat) = esta embarcação.
  const { data: conversa } = await supabaseAdmin
    .from('conversa')
    .upsert(
      {
        gestor_id: embarcacao.owner_id,
        cliente_id: user.id,
        origem_tipo: 'embarcacao',
        origem_id: id,
        origem_label: embarcacao.nome,
      },
  ...
  const rascunhoInicial =
    (mensagens ?? []).length === 0
      ? `Olá! Tenho interesse na embarcação "${embarcacao.nome}". Podemos conversar?`
      : undefined;
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 min-h-0">
        <ChatBox
          ...
          voltarHref={`/embarcacoes/${id}`}
          voltarLabel="Voltar para a embarcação"
